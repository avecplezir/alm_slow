...
class AlmAgent(object):
    def __init__(self, device, action_low, action_high, num_states, num_actions,
                ...
                log_wandb, log_interval, delay_actions=False
                ):

        self.device = device 
        self.action_low = action_low
        self.action_high = action_high
        self.delay_actions = delay_actions
    ...
    def get_action(self, state, step, hidden_activations, eval=False):
        ...
        with torch.no_grad():
            ...
            if self.delay_actions:
                action_dist, hidden_activations = self.actor.delay_forward(z, std, hidden_activations)
            else:
    ...
    def get_hidden_activations(self, state, step, eval=False):
        std = utils.linear_schedule(self.expl_start, self.expl_end, self.expl_duration, step)
        with torch.no_grad():
            state = torch.FloatTensor(state).unsqueeze(0).to(self.device)
            z = self.encoder(state).sample()
            action_dist, hidden_activations = self.actor(z, std, return_hidden_activations=True)

        # hidden_activations = [torch.zeros_like(h) for h in hidden_activations]
    ...
    def update_rest(self, std, step, log, metrics):
        ...
        #update actor
        self.update_actor(state_batch, z_dist.sample(), std, log, metrics)
    ...
    def update_actor(self, state_batch, z_batch, std, log, metrics):
        actor_loss = self._lambda_svg_loss(state_batch, z_batch, std, log, metrics)
    ...
    def _lambda_svg_loss(self, state_batch, z_batch, std, log, metrics):
        actor_loss = 0
    ...
    def _rollout_imagination(self, z_batch, std):
        ...
        with utils.FreezeParameters([self.model]):
            for t in range(self.seq_len):
                if self.delay_actions:
                    if t == 0:
                        action_dist, hidden_activations = self.actor(z_batch.detach(), std, return_hidden_activations=True)
                        # hidden_activations = [h.detach() for h in hidden_activations]
                        # action_dist, hidden_activations = self.actor.delay_forward(z_batch.detach(), std,
                        #                                                            hidden_activations)
                    else:
                        action_dist, hidden_activations = self.actor.delay_forward(z_batch.detach(), std, hidden_activations)
                else:
                    action_dist = self.actor(z_batch.detach(), std)
