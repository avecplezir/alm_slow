...
class MujocoWorkspace:
    def __init__(self, cfg):
        ...
        self.train_env.frame_skip, self.eval_env.frame_skip = self.cfg.frame_skip, self.cfg.frame_skip
        print('frame_skip train_env eval_env', self.train_env.frame_skip, self.eval_env.frame_skip)
    ...
    def train(self):
        ...
        hidden_activations = self.agent.get_hidden_activations(state, self._train_step)

        for _ in range(1, self.cfg.num_train_steps-self.cfg.explore_steps+1):  

            action, hidden_activations = self.agent.get_action(state, self._train_step, hidden_activations)
            next_state, reward, done, info = self.train_env.step(action)
            self._train_step += 1
    ...
    def _eval(self):
        ...
        for _ in range(self.cfg.num_eval_episodes):
            ...
            hidden_activations = self.agent.get_hidden_activations(state, self._train_step)
            while not done:
                action, hidden_activations = self.agent.get_action(state, self._train_step, hidden_activations, True)
